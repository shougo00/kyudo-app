...
@section('content')
@php
    $selectedUserIds = collect(old('recipient_ids', $selectedUserIds))->map(fn($id) => (int) $id)->all();
@endphp

<div class="news-admin-page py-2">
    <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3">
        <div>
            <h4 class="mb-1">お知らせ編集</h4>
            <div class="text-muted">内容や配信先を変更できます。</div>
        </div>
    ...
    <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data" class="news-form">
        ...
        <div class="news-form-panel">
            <div class="mb-3">
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $news->title) }}" class="form-control" required>
                @error('title')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
            </div>

            <div class="mb-3">
                <label class="form-label" for="body">本文</label>
                <textarea id="body" name="body" rows="7" class="form-control" required>{{ old('body', $news->body) }}</textarea>
            ...
            <div class="mb-3">
                <label class="form-label" for="image">画像を{{ $news->image_path ? '差し替え' : '追加' }}</label>
